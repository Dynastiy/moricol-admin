<template>
    <div
      id="app-drawer"
      class="tw-flex tw-flex-col tw-justify-between tw-pt-5 tw-py-4 tw-pb-12 tw-bg-primary-500 tw-z-20 tw-fixed tw-left-0 tw-top-0 tw-h-screen tw-overflow-auto tw-w-64"
    >
      <div>
        ...
        <ul class="tw-pl-0 tw-flex tw-flex-col tw-gap-2">
          <li v-for="(item, index) in menu" :key="index" class="first:tw-py-2 first:tw-border-b tw-border-white first:tw-border-b-2">
            <div class="menu-item ">
              <span
                role="button"
                class="tw-flex tw-justify-between tw-capitalize tw-text-semibold tw-block tw-py-5 tw-px-6 tw-font-medium tw-text-[14.4px] hover:tw-bg-gray200 hover:tw-text-dark-300"
                @click="item.hasChildren ? openSubMenu(item) : goToLink(item)"
                :class="[
                  item.parent === routeParent
                    ? 'tw-text-primary-500 tw-bg-white'
                    : 'tw-text-white',
                ]"
              >
